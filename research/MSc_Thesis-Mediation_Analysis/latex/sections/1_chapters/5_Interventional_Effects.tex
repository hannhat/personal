...
\begin{aligned}
	...
	\mathbb{P}[M_{2}(a'',m_{1}') = m_{2}|T = a''', M = m_{1}', C]\\
...
g-formula allows for identification of 22 different quantities $\Psi$ for a binary treatment $a$ 
...
\begin{enumerate}[noitemsep,topsep=0pt,label=(\roman*)]
	...
	for all $a \in \mathcal{T}$, $m_{1} \in \mathcal{M}_{1}$, $m_{2} \in \mathcal{M}_{1}$. 
...
$0 < \mathbb{P}[M_{2} = m_{2} | T = a, M = m_{1}, C]$
...
effect even in the presence of exposure-induced confounding. The distinctive feature of these effects
...
assigns a level of headache from the distribution of insomnia for patients who did take the 
anti-depressant. 
...
decompose the total interventional effect $\Phi(1,1,1,1) - \Phi(0,0,0,0)$, which is not always equal to the total
...
Arm 4 and Arm 4 and Arm 5 respectively; direct effects can be identified by comparing outcomes for Arms 6
...
effect of the anti-depressant on depressian via headache, we might prescribe a particular dose of a 
...
among practicioners attempting to conduct descriptive mediation analysis.~\citep{miles2023} 
...
settings with multiple mediators and exposure-induced confounding, which are very common in the social and 
...
is then what the causal effect of randomly assigning the children of manual laborers 
...
of managers versus the social class of children of manual labourers --- that is eliminated 
...
exposure-induced confounding versus two mediator problems discussed in 
...
For simplicitly, we assume that no other confounders exist. 
...
given that the set of controlled counfounders remains the same. 
...
only mediator and holding the set of controlled counfounders the same in addition to controlling for intelligence
(which would framed as an additional confounder here). 
