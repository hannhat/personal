\chapter{Preliminaries}\label{chapter:sample}
...
demonstrate the different types of effects we are interested in estimating for the rest of this
dissertation. We first review the potential outcomes framework for "simple"
causal inference problems with no mediation, where the goal is to infer the causal 
effect of a particular change in a treatment variable on an outcome variable. In later sections, we 
extend this framework to the case of mediation analysis with one mediator and discuss the 
various mediational effects that have been proposed in the literature. 
...
$Y_{i}(a) = Y_{i}$ if unit $i$ receives treatment level $a \in \mathcal{A}$. 
\end{assumption}
...
outcomes: $Y_{i}(a) \indep A_{i} | C_{i} = c$ for all $a \in \mathcal{A}$, where 
$0 < \mathbb{P}[A_{i} = a | C_{i}] < 1$ for all $a \in \mathcal{A}$. 
\end{assumption}
